...
#[derive(Debug)]
pub struct OrderFreshRightVariablesRule {
    vars: Vec<VarIdx>,
}

impl OrderFreshRightVariablesRule {
...
impl Rule for OrderFreshRightVariablesRule {
    #[inline]
    fn compute(
        &mut self,
        context: &mut Context,
        ...
        {
            transitivity_context.fresh_right = self.vars.clone();
            Ok(vec![])
        } else {
