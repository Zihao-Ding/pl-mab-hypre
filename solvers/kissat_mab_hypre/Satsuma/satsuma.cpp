...
int commandline_mode(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "__HELP" || arg == "_H") {
            ...
            std::clog << "   "  << std::left << std::setw(23) <<
                      "--generator-support-min [N]" << std::setw(16) <<
                      "Keep only remainder generators with support >= N (default 4)" << std::endl;
            std::clog << "   "  << std::left << std::setw(23) <<
                      "--generator-support-max [N]" << std::setw(16) <<
                      "Keep only remainder generators with support <= N (default 64)" << std::endl;
            std::clog << "   "  << std::left << std::setw(23) <<
                      "--generator-top-k [N]" << std::setw(16) <<
                      "Keep only the top-k remainder generators by support (default 128)" << std::endl;
            std::clog << "   "  << std::left << std::setw(23) <<
                      "--generator-clause-budget [N]" << std::setw(16) <<
                      "Cap generic lex-leader generation by estimated clauses (default 50000)" << std::endl;
            std::clog << "   "  << std::left << std::setw(23) <<
                      "--disable-remainder-generators" << std::setw(16) <<
                      "Skip generic remainder generators after detection" << std::endl;
            return 0;
        ...
        } else if (arg == "__GENERATOR_SUPPORT_MIN") {
            if (i + 1 < argc) {
                i++;
                satsuma_preprocessor.set_generator_support_min(atoi(argv[i]));
            } else {
                std::cerr << "--generator-support-min option requires one argument." << std::endl;
                return 1;
            }
        } else if (arg == "__GENERATOR_SUPPORT_MAX") {
            if (i + 1 < argc) {
                i++;
                satsuma_preprocessor.set_generator_support_max(atoi(argv[i]));
            } else {
                std::cerr << "--generator-support-max option requires one argument." << std::endl;
                return 1;
            }
        } else if (arg == "__GENERATOR_TOP_K") {
            if (i + 1 < argc) {
                i++;
                satsuma_preprocessor.set_generator_top_k(atoi(argv[i]));
            } else {
                std::cerr << "--generator-top-k option requires one argument." << std::endl;
                return 1;
            }
        } else if (arg == "__GENERATOR_CLAUSE_BUDGET") {
            if (i + 1 < argc) {
                i++;
                satsuma_preprocessor.set_generator_clause_budget(std::stoull(argv[i]));
            } else {
                std::cerr << "--generator-clause-budget option requires one argument." << std::endl;
                return 1;
            }
        } else if (arg == "__DISABLE_REMAINDER_GENERATORS") {
            satsuma_preprocessor.set_use_remainder_generators(false);
    ...
    bool ans = kissat_simplify(filename, formula, entered_file, proof_filename);
    if (!ans)
        return 1;
...
int main(int argc, char *argv[]) {
    int res = commandline_mode(argc, argv);
    if (res == 1){
