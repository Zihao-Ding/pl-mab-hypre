...
public:
    ...
    void delete_generator_at(int index) {
        delete generators[index];
        generators[index] = nullptr;
    }

    void retain_generators_by_index(const std::vector<int>& keep_indices) {
        dejavu::ds::markset keep(generators.size());
        for(const int index : keep_indices) {
            if(index >= 0 && index < static_cast<int>(generators.size())) keep.set(index);
        }

        std::vector<dejavu::groups::stored_automorphism*> filtered;
        filtered.reserve(keep_indices.size());
        for(int i = 0; i < static_cast<int>(generators.size()); ++i) {
            if(keep.get(i)) filtered.push_back(generators[i]);
            else delete_generator_at(i);
        }
        generators.swap(filtered);
    }

    void clear_generators() {
        for(int i = 0; i < static_cast<int>(generators.size()); ++i) {
            delete_generator_at(i);
        }
        generators.clear();
    }

    std::vector<generator_info> collect_generator_info() {
        std::vector<generator_info> info;
        info.reserve(generators.size());

        for(int i = 0; i < static_cast<int>(generators.size()); ++i) {
            aw.reset();
            generators[i]->load(aw);
            info.push_back({i, aw.nsupp()});
        }
    ...
    void filter_generators_by_support_topk(int support_min = 0, int support_max = -1, int top_k = -1) {
        if(generators.empty()) return;

        std::vector<generator_info> info = collect_generator_info();
        std::vector<int> keep_indices;
        keep_indices.reserve(info.size());

        std::stable_sort(info.begin(), info.end(), [](const generator_info& left, const generator_info& right) {
            return left.support < right.support;
        });

        for(const generator_info& entry : info) {
            if(entry.support < support_min) continue;
            if(support_max >= 0 && entry.support > support_max) continue;
            if(top_k >= 0 && static_cast<int>(keep_indices.size()) >= top_k) break;
            keep_indices.push_back(entry.index);
        }

        std::sort(keep_indices.begin(), keep_indices.end());
        retain_generators_by_index(keep_indices);
    }
    ...
    int add_lex_leader_for_generators(abstract_formula& formula, predicate& sbp, int depth = 50,
                                      uint64_t clause_budget = std::numeric_limits<uint64_t>::max()) {
        int constraints_added = 0;
        uint64_t clauses_reserved = 0;
        ...
        //for(int j = generators.size()-1; j >= 0; --j) {
            aw.reset();
            generators[j]->load(aw);
            const uint64_t estimated_clauses = sbp.estimate_lex_leader_clause_count(aw, depth);
            if(clauses_reserved + estimated_clauses > clause_budget) break;
            clauses_reserved += estimated_clauses;
