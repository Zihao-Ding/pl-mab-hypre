...
if [ "$N" -lt 5000000 ]; then
    ...
    if [ $SATSUMA_EXIT -eq 0 ]; then
        ./kissat "$temporary_cnf" "$proof_file" --time=5000 --no-binary -q --witnessmaxvar="$N" 2>&1 | tee >(grep '^v' > "$sat_file") | cat > /dev/null 2>&1
    fi
else
    echo "pseudo-Boolean proof version 3.0" > "$proof_file"
    ./kissat "$input_file" "$proof_file" --time=5000 --no-binary -q --witnessmaxvar="$N" 2>&1 | tee >(grep '^v' > "$sat_file") | cat > /dev/null 2>&1
fi
