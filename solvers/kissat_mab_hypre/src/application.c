                          ...
                          char **argv)
{
  ...
  solver->next_heuristic[0] = 1 - GET_OPTION(heuristic);
  solver->next_heuristic[1] = GET_OPTION(heuristic);
  solver->heuristic = GET_OPTION(heuristic);
  solver->mab = GET_OPTION(mab);
  if (solver->mab)
  {
    for (unsigned i = 0; i < 2 * solver->mab_heuristics; i++)
    {
                           ...
                           bool *cancel_alarm_ptr)
{
  *cancel_alarm_ptr = false;
  ...
  {
    FILE *rf = fopen("result.txt", "a+");
    fprintf(rf, "%s\t-1\t%.3f\n", argv[1], kissat_time(solver));
    fclose(rf);
#ifndef NPROOFS
