...
int kissat_analyze (kissat *solver, clause *conflict) {
  ...
  do {
    ...
    if (!EMPTY_STACK (solver->analyzed)) {
      ...
      // AW-VSIDS: Update activity tracking with learned clause info
      if (GET_OPTION (awvsids) && solver->variable_activities_ext) {
        unsigned glue = SIZE_STACK (solver->levels);
        kissat_update_aw_vsids (solver, solver->analyzed.begin, 
                                (unsigned) SIZE_STACK (solver->analyzed), glue);
        solver->current_learned_glue = glue;
      }
