...
static void eliminate_variables (kissat *solver) {
  ...
  for (;;) {
    ...
    /* Cycle 6 bug fix: was `unsigned last_round_eliminated = 0;` which
       shadowed the function-scope declaration at the top. The shadow
       made the outer variable always stay 0, so the `complete` check
       at the end of the function (`!remain && !last_round_eliminated`)
       saw `!remain && !0 = !remain` — BVE was wrongly declaring the
       round "complete" (no eliminations) even when variables had been
       eliminated, pushing `set_next_elimination_bound` toward more-
       aggressive future bounds. Now we reset (assign) rather than
       redeclare, so the outer variable actually reflects this round's
       eliminations. */
