...
void kissat_increase_size (kissat *solver, unsigned new_size) {
  ...
  // AW-VSIDS: Allocate extended activity tracking
  if (GET_OPTION (awvsids)) {
    if (old_size == 0) {
      solver->variable_activities_ext = (variable_activity_extended_t *)
        kissat_calloc (solver, new_size, sizeof (variable_activity_extended_t));
    } else {
      NREALLOC_VARIABLE_INDEXED (variable_activity_extended_t, variable_activities_ext);
    }
  }
