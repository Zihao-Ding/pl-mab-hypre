...
#[derive(Debug)]
pub struct OrderLeftVariablesRule {
    vars: Vec<VarIdx>,
}

impl OrderLeftVariablesRule {
...
impl Rule for OrderLeftVariablesRule {
    #[inline]
    fn compute(
        &mut self,
        context: &mut Context,
        ...
        if let Some(Subcontext::Order(order_context)) = context.subcontexts.last_mut() {
            order_context.left_vars_defined = true;
            order_context.constructed_order.left_vars = self.vars.clone();
