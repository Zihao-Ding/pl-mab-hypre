...
/*
 * PASSAT is a dynamic local search implemented on YalSAT (https://doi.org/10.1007/978-3-031-57246-3_3). It is a more
...
class PASSAT : public LocalSearchInterface
{
  public:
	...
	/// @brief Returns a hashmap relatting each clause to its PASSAT weight
  ...
  private:
	...
	float PASSATWeightToTransfer(const float victimWeight);
  ...
  private:
	...
	/// @brief Stores the number of clauses in the formula fed to PASSAT
	...
	/// @brief Stores the indices of the picked variables in the inner array of PASSAT
