  ...
  *--end_of_buffer = 0;
  ...
  for (all_stack (int, elit, proof->line)) {
    ...
    if (elit < 0) {
      ...
      eidx = elit;
    write_char (proof, 'x'); //variable name
...
static void print_red_proof_line (proof *proof) {
  proof->lines++;
  // red statement
  write_char (proof, 'r');
  write_char (proof, 'e');
  ...
  *--end_of_buffer = 0;
  ...
  if(has_firstlit) {
      ...
      // write first literal name
      char *p = end_of_buffer;
      write_char (proof, 'x'); //variable name
      for (unsigned tmp = firstlit_idx; tmp; tmp /= 10)
        *--p = '0' + (tmp % 10);
      while (p != end_of_buffer)
        write_char (proof, *p++);
...
void kissat_puts(proof *proof, char *st)
{
    for (char *p = st; *p; p++) {
        write_char(proof, *p);
    }
...
void kissat_putint(proof *proof, long long x)
{
    if (x < 0) {
        write_char(proof, '-');
        x = -x;
