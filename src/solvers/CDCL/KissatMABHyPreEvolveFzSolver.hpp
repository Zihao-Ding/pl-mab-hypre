...
#define KISSAT_MAB_HYPRE_EVOLVE_

// Kissat includes
extern "C"
{
#include <kissat_mab_hypre_evolve_fz/src/kissat.h>
}

/// Instance of a Kissat MAB Hybrid Presolve solver
/// @ingroup solving_cdcl
class KissatMABHyPreEvolveSolver : public SolverCdclInterface
{
  public:
	/// Constructor.
	KissatMABHyPreEvolveSolver(int id, const std::shared_ptr<ClauseDatabase>& clauseDB);

	/// Destructor.
	virtual ~KissatMABHyPreEvolveSolver();
  ...
  protected:
	...
	friend int kissatMABHyPreTerminate(void* solverPtr);
	 ...
	 * destruction) */
	friend char kissatMABHyPreImportClause(void*, kissat*);
	friend char kissatMABHyPreExportClause(void*, kissat*);
};